...
public class ProntuarioBS extends HibernateBusiness{
	...
	public void anamneseGeral(AnamneseGeral anamnese) {
		if(this.anamneseGeral(anamnese.getAnimal().getId()) == null) {
			this.dao.persist(anamnese);
		}else {
			AnamneseGeral anamesegeral = this.anamneseGeral(anamnese.getAnimal().getId());
	...
	public void exameFisico(ExameFisico exame) {
		if(this.exameFisico(exame.getAnimal().getId()) == null) {
			this.dao.persist(exame);
		}else {
			ExameFisico exameFisico = this.exameFisico(exame.getAnimal().getId());
	...
	public void resultados(Resultados resultados) {
		if(this.resultados(resultados.getAnimal().getId()) == null) {
			dao.persist(resultados);
		}else {
			Resultados result = this.resultados(resultados.getAnimal().getId());
